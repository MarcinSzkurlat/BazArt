...
import { Button, Icon, Label, Popup, Image, Grid, Header } from "semantic-ui-react";
...
export default observer(function FavoriteProductItem({ product }: Props) {
    const { productStore } = useStore();

    const handleRemoveFavoriteButton = () => {
        productStore.deleteFavoriteProduct(product.id);
    }
    ...
    return (
        <Grid columns={3}>
            <Grid.Column width={5} textAlign='center'>
                ...
                <Image size='medium' as={Link} to={`/product/${product.id}`} src={product.imageUrl} alt={product.name} />
            </Grid.Column>
            <Grid.Column width={9} style={{position:'relative'} }>
                <Header as={Link} to={`/product/${product.id}`}>{product.name}</Header>
                    <p><i>{product.description.length > 200
